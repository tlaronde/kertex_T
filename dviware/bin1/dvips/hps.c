 ...
 */
 /* Modified 2019-02-15 by Thierry LARONDE <tlaronde@polynum.com> for
	fixed size buffers (200) that were too short for some strings
	causing memory violation.
  */
#include "dvips.h"
...
Boolean POPPED = 0;
Boolean PUSHED = 0;
Boolean NEED_NEW_BOX;
Boolean PAGEUS_INTERUPPTUS;
...
/* #define PAGESIZE 792 */
#define PAGESIZE (vpapersize / 65536) /* TL 2017-08-18 */
#define MARGIN 72 /* dvips default margin [TL] */
#define VERTICAL 0 /* boolean [TL] */
#define HORIZONTAL 1 /* boolean [TL] */
/* #define FUDGE 2.0 */
/* TL 2017-08-18 */
#define VFUDGE ( (curfnt != NULL) ?\
	(curfnt->scaledsize*mag/1000)/(alpha*0x100000)/2.0 : 2.0)
#define HFUDGE 2.0 /* TL 2017-08-18 */
...
static struct nlist * install_link(char *name, Hps_link *defn, int type);
static char * dup_str(char *w);
static Hps_link * link_dup(Hps_link *s);
static double dvi_to_hps_conv(int i, int dir);
static int vert_loc(int i);
static Hps_link * dest_link(char *s);
static int count_targets(void);
static void do_targets(void);
static void do_target_dict(void);
static int href_name_match(char *h, char *n);
static void stamp_hps(Hps_link *pl);
static void stamp_external(char *s, Hps_link *pl);
...
if ( tolower((unsigned char)*hs) == 'a') { /* valid start */
  POPPED = false;
  if (inHTMLregion) {
...
/* parse for names and href */
  ...
  skip_space(hs);
	 ...
	 } else if ( (*hs == '/') && (tolower((unsigned char)hs[1]) == 'a') ) {
				  if (!inHTMLregion) {
	  ...
	  if ((nl = lookup_link(current_name, current_type)->defn)) { 
		 nl->rect.urx = dvi_to_hps_conv(hh + hoff,HORIZONTAL) + HFUDGE;
		 nl->rect.ury = dvi_to_hps_conv(vv + voff, VERTICAL)+ VFUDGE; 
...
if (HREF_COUNT++ > 0) {
  ...
  p->title = s;
  p->srcpg = pagecounter;
  p->rect.llx = dvi_to_hps_conv(hh + hoff,HORIZONTAL) - HFUDGE; 
  p->rect.lly = dvi_to_hps_conv(vv + voff,VERTICAL) - VFUDGE;
  p->rect.urx = -1.0;
  p->rect.ury = -1.0;
  p->vert_dest = -1; 
  p->page = -1;
...
stamp_external(char *s, Hps_link *pl)
{
  if (s == NULL || pl == NULL) {
	...
	bufsize = strlen(s) + 256;
	if ( (tmpbuf = malloc(bufsize)) == NULL )
	 	error("! no memory for HPS");

	 /* print out the proper pdfm with local page info only 
	  *  target info will be in the target dictionary */
	 (void)snprintf(tmpbuf, bufsize, " [[%.0f %.0f %.0f %.0f] [%i %i %i [%i %i]] [%.0f %.0f %.0f]] (%s) pdfm ", pl->rect.llx, pl->rect.lly, pl->rect.urx, pl->rect.ury,
		  pl->border[0], pl->border[1], pl->border[2], pl->border[3],pl->border[4],
...
finish_hps(void)
{
  ...
  noprocset = 1;
  removecomments = 0;
  copyfile("head.tmp", 0U);
  copyfile("body.tmp", 0U);
}
...
/* The box size is as is except for a constant 2.0 shift in vertical
	(in the original version; unchanged [TL 2017-08-18]
 */
void
...
{ 
  ...
  if (!current_rect_list) {
	 ...
	 current_rect_list->rect.lly = dvi_to_hps_conv(vv + voff, VERTICAL) - 2.0;
	 current_rect_list->rect.urx = dvi_to_hps_conv(hhmem, HORIZONTAL);
	 current_rect_list->rect.ury = dvi_to_hps_conv(vvmem, VERTICAL) - 2.0; 
...
{ 
  ...
  if (current_type == HREF && current_name[0] != '#') {
	 if ((nl = lookup_link(current_name, current_type)->defn)) { 
		nl->rect.urx = dvi_to_hps_conv(hhmem + hoff, HORIZONTAL) + HFUDGE;
		nl->rect.ury = dvi_to_hps_conv(vvmem + voff, VERTICAL) + VFUDGE; 
		 stamp_external(current_name,nl); /* Broken lines ? */
	  } else {
		 error("!Null lookup");
		 }
