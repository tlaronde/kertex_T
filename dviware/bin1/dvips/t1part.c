...
#define C1			  52845	/* eexec encoding; cf. T1-spec */
#define C2			  22719
#define EER			 55665	
#define CDR			  4330
...
unsigned char basevect[] = ADOBE_ENCODING;
...
/* number must not exceed NUM_LABEL [TL] */
int  number;
#define INCR_NUMBER\
	if (++number > NUM_LABEL) {\
		(void) fprintf(stderr, "number > %d! Increase NUM_LABEL!\n", NUM_LABEL);\
		exit(1);}
...
static FILE *psfopen(const unsigned char *name, unsigned int pathspec);
...
static unsigned char CDeCrypt(unsigned char cipher, unsigned int *lcdr);
...
static int Afm(void);
static int LoadVector(int num, CHAR *TmpChar);
static int ChooseVect(CHAR * tmpChar);
static void ErrorOfScan(int err);
static void NameOfProgram(void);

FILE *
psfopen(const unsigned char *name, unsigned int pathspec)
{
	FILE *fp;

	nameSet((const char *)name);
	(void)Openin(&fp, pathspec, "r");
	return fp;
}
...
DefTypeFont(unsigned char *name)
{
	 ...
	 {
		  if(strstr((const char *)name,TypeFonts[i].extension)!=0)
...
GetWord(unsigned char *mem)
{
	 ...
	 *tmp= *line;
	 while((*line!='\0')&&(*line!='%'))
	 {
		  ...
		  if(isalpha(*line))
		  {
				while(!isspace(*line) && *line != '/') /* '/' is a token delimiter too */
					 *tmp++= *line++;
				*tmp = '\0';
				return(token_type=token_type+2);
		  }
...
GetNum(void)
{
	 ...
	 *tmp= *temp;
	 ...
	 while(temp<end_of_scan)
	 {
		  if(isdigit(*temp))
		  {
				while(isdigit(*temp) || *temp == '.') /* integer or real */
					 *tmp++= *temp++;
				*tmp = '\0';
				return(atoi((const char *)token));
		  }
		  temp++;
	 }
...
AddChar(CHAR *TmpChar, unsigned char *CharName, int num)
{
	 ...
	 length			= strlen((const char *)CharName);
	 ThisChar->name = getmem(length+1);
	 strcpy((char *)ThisChar->name, (const char *)CharName);
...
AddStr(unsigned char *name, int num)
{
	 ...
	 strcpy((char *)ThisStr->name, (const char *)name);
	 ThisStr->num=num;
...
ChooseChar(unsigned char *name, CHAR *TmpChar)
{
	 ...
	 while (ThisChar != NULL)
	 {
		  ...
		  {
				if (strcmp((const char *)name, (const char *)ThisChar->name) == 0)
				{
					 ThisChar->choose=1;
					 return  1;
				}
 ...
 */
		  if (NextChar == 0) {
			  ...
			  strcpy((char *)NextChar->name, (const char *)name);
...
ScanSubrs(int i)
{
	 ...
	 for(; number <  keyword[i].oldnum + keyword[i].offset;)
	 {
		  if((word_type=GetToken())>0)
		  {
				if(word_type==2)
				{
					 if(!strcmp((const char *)token,Dup))
					 {
						  if(test==0)
								test=1;
...
void
ViewReturnCall(int num_err, int top, int *pstack, int j, int depth)
{
	 int k,m;
...
ScanChars(int i)
{
	 ...
	 while( number < max_num )
	 {
		  if((word_type=GetToken())>0)
		  {
				if(word_type>=3)
				{
					 strcpy((char *)tmp_token, (const char *)token);
					 str_len = strlen((const char *)token);
					 ...
					 if(CharCount!=0)
					 {
						  ...
						  else
						  {
								...
								if(word_type>3)
								{
									 if(strstr((const char *)token, (const char *)notdef)!=NULL)
									 {
										  CharCount--;
										  label[number].num = -2;
										  found=FLG_BINARY;
					 ...
					 {
						  ErrorOfScan(num_err);
						  fprintf(stderr,"in Char string of '%s'\n", tmp_token);
						  exit(1);
					 }
					 INCR_NUMBER;
				}
	 ...
	 for(;;)
	 {
		  if((word_type=GetToken())>0)
		  {
				if(word_type==3)
				{
					 ...
					 {
						  if(!strcmp((const char *)token, Key[i].name))
						  {
								tmp_num = GetNum();
								if(tmp_num<0)
								{
								...
								keyword[current].length=strlen((const char *)token);
...
EndOfEncoding(int err_num)
{
	 ...
	 static char *RefKey[] = /* the words for which we CorrectGrid() */
	 {
		 "readonly",
		 "getinterval",
		  "exch",
	 ...
	 for(;;)
	 {
		  ...
		  if(err_num==5)
				refer[ind_ref].num[i++]=atoi((const char *)token);
		  else
		  {
				for(j=0; *RefKey[j]; j++)
				{
					  if(strcmp((const char *)token, RefKey[j]) ==0)
								break;
				}
				switch(j)
				{
				...
				case 3:
					...
					case 2:
						  if(i==1)
						  {
								...
								GetWord(token);
								refer[ind_ref].num[0]= atoi((const char *)token);
						  }
						  i=0;
...
CharEncoding(void)
{
	 ...
	 if(err_token==2)
	 {
		  if(strcmp((const char *)token, Dup) ==0)
		  {
				err_token=GetWord(token);
				if(err_token<0)
					 return ERR_NUM_CHAR;
...
FindEncoding(void)
{
	 ...
	 if(encode==0)
	 {
		  ...
		  while((num_err=GetWord(token))>=0)
		  {
				if(num_err==3)
				{
					 if (strcmp((const char *)token, "/Encoding") == 0)
					 {
...
/* It's eexec decryption for PFB format.
	A whole string is decrypted, but since the key varies, it has to
	be global. [TL]
 */
void
...
/* And  it's eexec decryption for PFA format */
void
...
PartialPFA(FILE *fin, FILE *fout)
{
	 ...
	 for(;;)
	 {
		  ...
		  {
				case FLG_ASCII:
					 if(strstr((const char *)buf,"currentfile eexec") != NULL)
					 {
						  type=FLG_BINARY;
					 }
				...
				case FLG_BINARY:
					 ...
					 add_of_len=strlen((const char *)buf)/2;
...
/* of memory  but it will be more slowly	 */
						  begin_of_scan = realloc(begin_of_scan, memory);
...
PartialPFB(FILE *fin, FILE *fout)
{
	 ...
	 line=tmpline;
	 edr  = EER;	/* initialize for eexec decryption */
...
Afm(void)
{
	 ...
	 if ((fafm = psfopen(afmfile, FONTSPATHSPEC | AFMSUBDIR)) == NULL)
	 {
		  NameOfProgram();
		  perror((const char *)afmfile);
		  return -1;
	 }
	 ...
	 for(j=0;;)
	 {
		  ...
		  if(strstr((const char *)line, AfmKey[j])!=NULL)
		  {
				if(j==0)
				{
					 j++;
		  ...
		  if(j==1)
		  {
				for(k=0; err_num>=0; )
				{
					 ...
					 if(err_num==2)
					 {
						  if(strcmp((const char *)token,InfoKey[k])==0)
						  {
								if(k==0)
								{
									 err_num=GetWord(token);
								...
								else
								{
									 ...
									 name[0]='/';
									 name[1]='\0';
									 strcat((char *)name,(const char *)token);
									 if(num>=0)
...
int
FontPart(FILE *fout, unsigned char *fontfile, unsigned char *vectfile)
{
	 FILE  *fin=0;
	 int	num;
	 int	rc;
	 ...
	 keep_flg=0;
	 flg_seac=0;
	 strcpy((char *)psfontfile, (const char *)fontfile);
	 find_encod=0;
	 CharCount=0;
	 ...
	 if(loadbase != 1)
	 {
		  ...
		  strcpy((char *)psvectfile, (const char *)basevect);
	 ...
	 if(vectfile)
	 {
		  ...
		  strcpy((char *)psvectfile, (const char *)vectfile);
	 }
	 ...
	 {
		  case PFA:
				if ((fin = psfopen(fontfile, FONTSPATHSPEC | PFASUBDIR))==NULL)
				{
					 NameOfProgram();
				...
				break;
		  case PFB:
				if ((fin = psfopen(fontfile, FONTSPATHSPEC | PFBSUBDIR))==NULL)
				{
					 NameOfProgram();
					 perror((const char *)fontfile);
					 return -1;
				}
...
LoadVector(int num, CHAR *TmpChar)
{
	 ...
	 if ((fvect = psfopen(psvectfile, FONTSPATHSPEC | ENCSUBDIR)) == NULL)
	 {
		  NameOfProgram();
		  perror((const char *)psvectfile);
		  return -1;
	 }
