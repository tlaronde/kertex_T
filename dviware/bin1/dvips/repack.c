...
static void putlong(register char *a, long i);
static void addts(register unsigned char what);
static void dochar(unsigned char *from, short width, short height);
static char * makecopy(register unsigned char *what, register long len,			register unsigned char *p);
...
addts(register unsigned char what)
{
	...
	if (tsp >= tsend) {
		if (tempstore == NULL) {
			tslen = 2020;
			tempstore = (unsigned char *)mymalloc(tslen);
			tsp = tempstore;
		} else {
			tslen = 2 * tslen;
			tsp = (unsigned char *)mymalloc(tslen);
...
dochar(unsigned char *from, short width, short height)
{
	...
	if (widthc > rowlength) {
		if (rowlength > 0)
			free((char *)specdata);
		rowlength = widthc + 30;
		specdata = (unsigned char *)mymalloc((size_t)(rowlength + 15));
	}
...
char *
makecopy(register unsigned char *what, register long len,
	register unsigned char *p)
{
	...
	if (p == NULL) {
		if (len > MINCHUNK)
			p = (unsigned char *)mymalloc((size_t)len);
		else {
			if (bytesleft < len) {
...
repack(register chardesctype *cp)
{
	...
	if (mbytesleft < i) {
		...
		if (RASTERCHUNK > i) {
			mraster = (quarterword *)mymalloc(RASTERCHUNK);
			mbytesleft = RASTERCHUNK;
		} else {
			i += i / 4;
			mraster = (quarterword *)mymalloc((size_t)(i + 3));
			mbytesleft = i;
		}
	}
	while (i > 0)
