...
static integer nscalewidth(register integer a, register integer b);
static void bbtfmload(int n);
static void bbspecial(integer h, integer v, int nbytes);
static void bbdopage(void);

integer
nscalewidth(register integer a, register integer b)
{
	if (a < 0)
...
/* no marks on the page? */
	if (llx >= urx || lly <= ury)
		llx = lly = urx = ury = 72;
	sprintf(nextstring, "%ld %ld %ld %ld", (long)llx, (long)ury, (long)urx, (long)lly);
}
