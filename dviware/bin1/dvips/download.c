...
static void downchar(chardesctype *c, shalfword cc);
static void downpsfont(charusetype *p, charusetype *all);
	...
	if (compressed) {
		...
		if (mbytesleft < i) {
			...
			if (RASTERCHUNK > i) {
				...
				k = i + i / 4;
				mraster = (quarterword *)mymalloc((size_t)k);
				mbytesleft = k;
			}
		}
		k = i;
...
downpsfont(charusetype *p, charusetype *all)
{
	...
	copyfile(all->fd->resfont->Fontfile, FONTSPATHSPEC | PFBSUBDIR);
		  infont = 0;
	return;
	 }
	 for (cc=0; cc<256; cc++)
...
/*		tmpgrid[cc]=grid[cc]; */
	 ...
	 if(GridCount!=0) {
		  if (!quiet) {
			  if (strlen(rf->Fontfile) + prettycolumn > LINELENGTH) {
				  fprintf(stderr, "\n");
				  prettycolumn = 0;
			  }
		...
		prettycolumn += strlen(rf->Fontfile) + 2;
		  ...
		  if(FontPart(bitfile, (unsigned char *)rf->Fontfile, (unsigned char *)rf->Vectfile) < 0)
				exit(1);
		  if (! disablecomments)
