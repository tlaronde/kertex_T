...
static void InstallPL(integer pslow, integer pshigh);
...
InstallPL(integer pslow, integer pshigh)
{
	 ...
	 pl = (struct p_list_str *)mymalloc(sizeof *pl);
	 pl -> next = ppages;
	 pl -> ps_low = pslow;
	 pl -> ps_high = pshigh;
	 ppages = pl;
