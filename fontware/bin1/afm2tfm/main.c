 ...
 *   Both files does not restrict the font to the first 128 integers
 *   (ASCII range).
 */
 ...
 */
/* C89 cleaning for kerTeX.
   Cleaning of unused and unmentionned '-P' plus searching for encoding
   files, no hard-coded enc, but a default name for an external file,
   etc.
   Thierry Laronde 2010-04-06/2011-05.
   2022-03-12 new WEBLIB API for file opening [TL].
 */
/* This is the default output encoding used if none specified.
   Compatible with what was hard coded in the file. [TL]
 */
#define DEFAULT_OUT_ENCODING "default.enc"
...
char *inname, *outname; /* names of input and output files */
char buffer[filenamesize+1] ; /* input buffer (modified while parsing) */
char obuffer[filenamesize+1] ; /* unmodified copy of input buffer */
...
double newslant ;
#define TITLEBUF_SIZE 500
char titlebuf[TITLEBUF_SIZE] ;

static void error(register char *s);
static int transform(register int x, register int y);
static int getline(void);
static int interest(char *s);
static char * mymalloc(size_t len);
...
static struct adobeinfo * findadobe(char *p);
static void handlekern(void);
static void handleconstruct(void);
static void makeaccentligs(void);
...
static void makebcpl(register long *p, register char *s, register int n);
...
static int texheight(register struct adobeinfo *ai);
...
static struct encoding *readencoding(char *enc);
...
getline(void) 
{
   ...
   param = buffer ;
   for (p=buffer; (p - buffer) < filenamesize
	  && (c=getc(afmin)) != EOF && c != '\n';)/* changed 10 to '\n' */
      *p++ = c ;
   *p = 0 ;
...
void
handleconstruct(void)
{ /* an input line beginning with CC */
   register struct adobeinfo *ai ;
   register char *p ;
   register struct pcc *np ;
   register int n ;
...
handlereencoding(void)
{
   ...
   ignoreligkern = 0 ;
	if (outenname == NULL)
		outenname = DEFAULT_OUT_ENCODING;
 ...
 */
   ...
   if (pedantic)
		return; /* XXX look afterward about finishup [TL] */
/*
 ...
 */
   for (ai=adobechars; ai; ai=ai->next)
      if (ai->texnum >= 0 && ai->texnum < 256) {
         j = -1 ;
		 /* XXX: so -1 will be set for first nexttex[ai->texnum];
		  * this slips in code when publishing! To be audited! [TL]
		  */
...
/* JLH: changed some lines below to be ascii<->ebcdic independent
   ...
   for (Ai=adobechars; Ai; Ai=Ai->next) {
      p = (unsigned char *)Ai->adobename ;
      if (isupper (*p)) {
         q = (unsigned char *)lwr ;
         for (; *p; p++)
            *q++ = (unsigned char) tolower(*p);
...
int
mincover(long *what, register long d)
{
   register int m ;
   register long l ;
   register long *p ;
...
long *header, *charinfo, *width, *height, *depth, *ligkern, 
	*kern, *tparam, *italic ;
long *tfmdata ;
...
vchar(int c)
{
	if (forceoctal == 0 && isalnum (c))
		(void) snprintf(vcharbuf, 6, "C %c", c);
	else {
		if (c < 0 || c > 255)
			(void)fprintf(stderr, "Char number out of range (%d)!\n", c);
		(void) snprintf(vcharbuf, 6, "O %o", (unsigned)c) ;
	}
   return (vcharbuf) ;
}

#define VNAMEBUF_SIZE 255
char vnamebuf[VNAMEBUF_SIZE];
char *
vname(int c)
{
  if (!forceoctal && isalnum (c)) {
    ...
    (void)snprintf(vnamebuf, VNAMEBUF_SIZE, " (comment %s)", texptrs[c]->adobename);
  }
  return vnamebuf;
}
...
writevpl(void)
{
   ...
   (void)snprintf(obuffer, filenamesize+1, "TeX-%s%s%s%s", outname,
   ...
   {
      char tbuf[300] ;
      char *base_encoding = codingscheme ;
      
      if (strcmp (outencoding->name, base_encoding) == 0) {
        (void)snprintf(tbuf, 300, "%s", outencoding->name);
      } else {
        (void)snprintf(tbuf, 300, "%s + %s", base_encoding, outencoding->name);
      }
               ...
               for (npcc = ai->pccs; npcc; npcc=npcc->next)
                  if (0 != (api=findadobe(npcc->partname)))
                     if (api->texnum>=0) {
						...
						/* XXX: -1? from where? */
						if (api->adobenum > 0 && api->adobenum < 256)
				...
				/* XXX: -1? from where? */
				if (ai->adobenum > 0 && ai->adobenum < 256)
					voutln2("(SETCHAR O %o)", (unsigned)ai->adobenum) ;
			}
            vright() ;
         }
         vright() ;
...
{
   (void)fprintf(f,
 "afm2tfm 8.2, Copyright 1990-97 by Radical Eye Software\n") ;
   (void)fprintf(f,
...
openfiles(int argc, char *argv[])
{
   ...
   (void)snprintf(titlebuf, TITLEBUF_SIZE, "%s %s", argv[0], argv[1]) ;
   /* allocate room including possible ".afm" suffix */
   inname = mymalloc(strlen(argv[1]) + 4 + 1); /* add room for suffix */
...
case 'V': makevpl++ ;
case 'v': makevpl++ ;
		 p = mymalloc(strlen(argv[3]) + 4 + 1); /* adds room for suffix */
		 free(outname); outname = p;
         ...
         if ((vplout=fopen(outname, "wb"))==NULL)
...
default: (void)fprintf(stderr, "Unknown option %s %s will be ignored.\n",
      ...
      for (i=0; i<arginc; i++) {
         (void)snprintf(titlebuf + strlen(titlebuf),
			TITLEBUF_SIZE - strlen(titlebuf), " %s", argv[2]) ;
         argv++ ;
         argc-- ;
      }
   }
	...
	nameSet(inname);
	if (Openin(&afmin, FONTSPATHSPEC | AFMSUBDIR, "rb") != true)
      error("! can't find or open afm input file") ;
   ...
   if (argc == 2) {
	  p = mymalloc(strlen(inname) + 4 + 1); /* room for suffix */
	  free(outname); outname = p;
      (void)strcpy(outname, inname) ;
   } else {
	  p = mymalloc(strlen(argv[2]) + 4 + 1); /* room for suffix */
	  free(outname); outname = p;
   ...
   if (tfmout == NULL && (tfmout=fopen(outname, "wb"))==NULL)
 ...
 */
   ...
   if (lastext) {
	  char *p, *q;
	  /* overlapping is caught by ssp __strcpy_chk */
	  p = outname;
	  q = outname + lastext;
	  while ((*p++ = *q++) != '\0')
		;
...
addkern(char *s1, char *s2)
{
  ...
  if (ai1 && ai2 && !ai1->kerns) {
    ...
    struct adobeptr *ap 
      = (struct adobeptr *) mymalloc(sizeof(struct adobeptr));
...
readencoding(char *enc)
{
   ...
   int i ;
   struct encoding *e =
      (struct encoding *)mymalloc(sizeof(struct encoding)) ;
   ...
   if (afmin)
	...
	nameSet(enc);
	(void)Openin(&afmin, FONTSPATHSPEC | ENCSUBDIR, "rb");
	param = 0 ;
	if (afmin == 0)
...
main(int argc_, char *argv_[])
{
   int i ;
	...
	argc = argc_;
	argv = argv_;
	setpaths(FONTSPATHSSET);	/* progname is set */
	...
	/* real afm2tfm processing starts now. [TL]
	 */
   for (i=0; i<256; i++)
      nexttex[i] = -1 ; /* encoding chains have length 0 */
   tfmdata = (long *)mymalloc(40000) ;
   openfiles(argc, argv) ;
   readadobe() ;
   if (fontspace == 0) {
      struct adobeinfo *ai ;
