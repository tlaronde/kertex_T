...
/* 2018-08-09: modified by T. Laronde <tlaronde@polynum.com> to
   suppress the optimization to not print out the newline after
   the first line of a tex block. This was "commenting" the material
   on the second line if the first one ended with a '%'.
   Some added comments and the handling of "\r\n" sequence for MS
   Windows.
   Note1: we expect ASCII characters (say in the future, utf8) and we
   do not use isalpha() to not enter the locale nightmare.
   Note2: the quoted strings shall not have embedded '%' and shall
 ...
 * material and it is surrounded by the [preamble] and [postamble] strings
...
char buf[BUFSIZ];	/* the input line */
char *bb, *tt, *aa;	/* start of before, token, and after strings */

static void usage(char *nam);
static void err(char *msg);
static char *getline(void);
static int match_str(char *s, char *t);
static int getbta(char *s);
static void copytex(void);
...
getline(void)	/* returns NULL on EOF or error, otherwise buf */
{
	...
	if (fgets(buf, BUFSIZ, mpfile)==NULL)
		return NULL;
	lnno++;
	for (s= &buf[1]; *s!='\0'; s++) /* do nothing */;
	if (s==&buf[BUFSIZ-1] && s[-1]!='\n') err("Line is too long");
	if ( (s-2) >= buf && s[-2] == '\r')
		s[-2] = '\0';
	else
		s[-1] = '\0';
 ...
 * b[efore], t[oken], a[fter], with the global pointers bb, tt, and aa
 * set to the start of the
 * corresponding strings.  String t is [a sentry,] either a quote mark,
 * a percent sign,
