/* This file adjusted for kerTeX 2011-04-06, 2022-03-07 by
   Thierry Laronde
...
static void quit(char *msg1,char *msg2,char *msg3);
static void warn(char *msg1,char *msg2,char *msg3);
static void add_to_pool(char c);
static int hash(register char *s);
static int *hfind(char *s, Hcell* htab);
static int hfound(void);
static int get_int(register char *s);
static float get_float(register char *s);
static void read_fmap(char *dbase);
static void read_char_adj(char *adjfile);
static void read_tfm(int f);
static int scan_desc_line(int f, char *lin);
static int read_fontdesc(char *nam);
static void print_char(char cc);
static void end_char_string(int l);
static void prepare_font_use(void);
static void first_use(int f);
static void slant_and_ht(void);
static void finish_last_char(void);
static void set_num_char(int f, int c);
static void start_picture(void);
static void stop_picture(void);
static int copy_spec_char(char *cname);
static void set_char(char *cname);
static void do_font_def(int n, char *nam);
static float Beval(float *xx, float t);
static float circangle(float t);
static float circtime(float a);
static void prepare_graphics(void);
static char * do_line(char *s);
static char * spline_seg(char *s);
static void do_ellipse(float a, float b);
static void do_arc(float cx, float cy, float ax, float ay, float bx, float by);
static void do_graphic(char *s);
static void change_font(int f);
static int do_x_cmd(char *s0);
...
char buf[filenamesize+1];	/* general buffer, large enough for paths */

int argc;	/* compatibility with libweb and pp2rc */
char **argv;
...
read_fmap(char *dbase)
{
    ...
    nfonts = 0;
	nameSet(dbase);
	if (Openin(&fin, INPUTSPATHSPEC, "rb") != true)
		quit("Unable to find file `", dbase, "'.");
    trfonts = new_htab;
...
read_char_adj(char *adjfile)
{
	...
	nameSet(adjfile);
	if (Openin(&fin, INPUTSPATHSPEC, "r") != true)
		quit("Unable to find file `", adjfile, "'.");
    for (i=0; i<nfonts; i++)
	shiftbase[i] = 0;
...
read_tfm(int f)
{
	...
	buf[0] = '\0';
	(void) strncpy(buf, texname[f], filenamesize - 4);
	buf[filenamesize - 4] = '\0';
	(void) strcat(buf, ".tfm");
	nameSet(buf);
	if (Openin(&tf, FONTSPATHSPEC|TFMSUBDIR, "rb") != true)
		quit("Unable to find file `", texname[f], "'.");
    for (i=0; i<5; i++) {
	sizes[i] = getc(tf);
...
read_fontdesc(char *nam)
{
	...
	nameSet(nam);
	if (Openin(&fin, FONTSPATHSPEC, "r") != true)
		quit("Unable to find troff font `", nam, "'.");
    for (;;) {
...
copy_spec_char(char *cname)
{
	...
	buf[0] = '\0';
	(void) strcpy(buf, "charlib" DIR_SEP);
	(void) strncpy(buf+8, cname, filenamesize - 8);
	buf[filenamesize] = '\0';
	nameSet(buf);
	if (Openin(&deff, INPUTSPATHSPEC, "rb") != true)
		quit("Unable to find charspec `", cname, "'.");
    while (k<speci) {
	if ((c=getc(deff))==EOF)
...
main(int argc_, char *argv_[])
{
	...
	argv = argv_;	/* compatibility with libweb */
	argc = argc_;
	{
	int i;
	for (i = 0; i < 256; i++)   
