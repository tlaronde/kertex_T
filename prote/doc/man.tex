...
(1)~When a primitive corresponds to a setting of a mode, affecting the
way \TeX\ behaves, the setting corresponding to defined values given
by symbolic names, the primitives are in fact setting a global
variable. These are |setting_*| variants.

(2)~When a primitive corresponds to a read-write variable, the variable
...
(3)~When the primitive corresponds to a read-only variable but whose
value can be expanded by |\the|, it is a |last_item| variant.

(4)~When the primitive expands to something that is externally given
(and is not expanded by |\the|), it is a |convert| variant.

(5)~When the primitive is a conditional, it is obviously a |if_else|
variant.

(6)~When the primitive changes (or may change) the definition of the
following token, it is a |expand_after| variant.

(7)~When the primitive is really orthogonal, it is an |extension|
variant.
...
representing a string, it is neither NUL nor space terminated: the
...
\begindiv Roff compatibility

@ \TeX\ is a layout engine. There is {\it de facto} no reason to
prevent \TeX\ from rendering texts described with {\tt roff} like
macros. One of the main differences concerning the macros is that
{\tt roff} treats specially a dot at the very start of a new line: it
introduces a macro call.

\Prote\ adds thus a global variable for the setting of the {\tt roff}
compatibility. Setting this variable with defined values---given as
symbolic names---is made {\it via} primitives.

This global variable (named |roff_compat| in \Prote, but this is an
implementation detail and is not part of the A.P.I.) behaves as
|interaction| in \TeX: it is not subject to grouping or local scope.

For now, there are only two values and hence two primitives:
|\noroffcompat| (this is the default) meaning no special treatment; and
|\roffdot|, treating a leading dot in a new line as an |escape|
command introducing a macro. For now and for future, any other
|roff| setting will validate this dot compatibility. (In \Prote,
|\noroffcompat| sets the global variable to $0$ and |\roffdot| sets
it to $1$, but the values have no meaning, only the order between the
values matters). Greater values will validate all lesser values
features, except of course the first corresponding to the 
|\noroffcompat| setting.

@ |\noroffcompat|. ({\sl global var |roff_compat| setting})\enskip
No {\tt roff} compatibility feature. This is the default.

@ |\roffdot|. ({\sl global var |roff_compat| setting})\enskip
A dot at the very beginning of a line (first character) is interpreted
as an escape command. Hence a macro generally called |\somecs| can
be called at the very beginning of a line as |.somecs|.

\enddiv % Roff compatibility
