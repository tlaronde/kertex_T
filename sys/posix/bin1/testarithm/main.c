...
/* Produces a 1-page summary of cpu times of various C constructs
...
main(int argc, char **argv)
{
	...
	setn(j1=ftakefraction(j2,j3),n);
	title("External arithmetic (#undef FIXPT)", n);
	loop("takefraction", j1=ftakefraction(j2,j3), n);
	loop("takescaled", j1=ftakescaled(j4,j4), n);
	loop("makefraction", j1=fmakefraction(j2,j3), n);
	loop("makescaled", j1=fmakescaled(j4,j4), n);
	exit(EXIT_SUCCESS);
}
